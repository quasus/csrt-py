...
from lib.utils import gaussian2d_rolled_labels, cos_window
...
def get_patch(img, center, scale, template_size):
...
def get_csr_features(img, center, scale, template_sz, resize_sz, cell_size):
    ...
    patch = cv2.getRectSubPix(img, patchSize=(int(scale*template_sz[0]),
...
class CSRDCF(BaseCF):
    def __init__(self, config):
        ...
        self.scale_config = config['scale_params']
    ...
    def init(self,first_frame,bbox):
        ...
        self.rescale_ratio = np.sqrt((self.params['template_size']**2)/
        ...
        if self.use_segmentation:
            ...
            mask=self.target_dummy_mask
        # extract features
        f = get_csr_features(first_frame,self._center,self.current_scale_factor,
    ...
    def create_csr_filter(self,img,Y,P):
        ...
        while True:
            ...
            if iter >= self.params['admm_iterations']:
                break
            L+=mu*(G-H)
